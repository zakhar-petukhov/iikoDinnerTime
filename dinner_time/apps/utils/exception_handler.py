...
def get_response(error=False, error_text=None, content=None, status_code=200):
    if content is None:
        content = {}
    ...
    def __call__(self, request):
        ...
        if response.status_code == 500:
            response = get_response(
                error=True,
                error_text="Internal server error, please try again later",
        ...
        if response.status_code == 404 and "Page not found" in str(response.content):
            response = get_response(
                error=True,
                error_text="Page not found, invalid url",
                status_code=response.status_code
            )
            return JsonResponse(response, status=response['status_code'])
